...
export default function VaporizeTextCycle({
  ...
  // Handle start and stop based on visibility
  useEffect(() => {
    if (isInView) {
      // Si solo hay un texto, nos quedamos en el estado estático de forma permanente para evitar ciclos infinitos molestos
      if (texts.length <= 1) {
...
const renderCanvas = ({
  ...
  // Draw them statically immediately so the canvas is not blank
  renderParticles(ctx, particles, globalDpr);
};
